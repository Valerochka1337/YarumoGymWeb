...
describe("trial draft", () => { it("round trips a versioned validated draft", async () => { expect(await saveDraft(state)).toBeUndefined(); expect((await loadDraft(state.token))?.operationId).toBe(state.operationId); }); it("reports storage failure without losing in-memory state", async () => { vi.spyOn(Storage.prototype,"setItem").mockImplementation(() => { throw new Error("quota"); }); expect(await saveDraft(state)).toMatch(/Не удалось/); }); });
